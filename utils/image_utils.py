...
def get_patch_grad(traj,H,W,device):
	aa_map = []
	aa = traj.reshape(-1, 2)
	for x, y in zip(aa[:, 0], aa[:, 1]):
		topographic_agent = create_topographic_map_torch(maxima_coords=(x, y), mask_shape=(H, W), level=2,
														 device=device)
		aa_map.append(topographic_agent)
	return aa_map

def get_patch_onehot(traj,H,W):
	aa_map = []
	device = traj.device
	aa = torch.round(traj.reshape(-1, 2)).type(torch.long).to(device)
	for x, y in zip(aa[:, 0], aa[:, 1]):
		canvas = torch.zeros(size=(H,W))
		canvas[y, x] = 1
		aa_map.append(canvas)
	return aa_map

def preprocess_image_for_segmentation(images, classes=2):
...
def sampling(probability_map, num_samples,scene, rel_threshold=None, replacement=False):
	probability_map = probability_map*(scene.int().unsqueeze(1))
...
def create_topographic_map_torch(maxima_coords, mask_shape, level, device='cpu'):
    shape = mask_shape
    x_coord, y_coord = torch.meshgrid(torch.linspace(0, shape[1] - 1, shape[1]).to(device),
                                      torch.linspace(0, shape[0] - 1, shape[0]).to(device))
    x_coord = torch.transpose(x_coord, 0, 1)
    y_coord = torch.transpose(y_coord, 0, 1)
    x, y = maxima_coords
    topographic_map = (torch.abs(x_coord - x) ** level + torch.abs(y_coord - y) ** level) ** (1 / level)
    topographic_map = normalize_torch(-topographic_map)
