...
class SameSoftArgmax2D(nn.Module):
    def __init__(self):
        super().__init__()
        self.softmax = nn.Softmax(dim=2)

    def img_to_indices_t(self, img):
        n, d = img.shape[0], img.shape[2]
        m = torch.argmax(img.view(n, -1), dim=1)
        indices = torch.stack((m // d, m % d), dim=1)
        indices = indices.flip(dims=[1])
        return indices

    def forward(self, x):
        softmax = self.softmax(x.view(*x.size()[:2], -1)).view_as(x)
        coord = [self.img_to_indices_t(softmax[i]) for i in range(softmax.shape[0])]
