...
class VideoTracker(object):
    ...
    def run(self):
        ...
        while self.vdo.grab():
            ...
            # mask = cv2.imread(f"./mask.png")
            # seg_1 = mask[:,:,0]
            # seg_2 = 255-mask[:,:,1]
            # a = np.stack([seg_1,seg_2])
            # cv2.imwrite("./mask_2.png",a)
            # mask = np.uint8(255 * mask)
            # mask = cv2.applyColorMap(mask, cv2.COLORMAP_JET)
            # img0 = cv2.addWeighted(img0, 0.7, mask, 0.3, 0)
...
if __name__ == '__main__':
    ...
    # input and output
    parser.add_argument('--input_path', type=str, default='./video/Sample.mp4', help='source')  # file/folder, 0 for webcam
